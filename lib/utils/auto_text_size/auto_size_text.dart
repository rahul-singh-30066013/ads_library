...
class _AutoSizeTextState extends State<AutoSizeText> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, size) {
        ...
        final fontSize = result.first as double;
        ...
        if (widget.group != null) {
          ...
          text = _buildText(widget.group?._fontSize ?? 1, style, maxLines);
        } else {
          text = _buildText(fontSize, style, maxLines);
        }

        return (widget.overflowReplacement != null && !textFits
  ...
  void _validateProperties(int? maxLines) {
    ...
    if (widget.presetFontSizes == null) {
      assert(
          widget.stepGranularity >= stepGranularity,
      ...
      assert(
        widget.presetFontSizes?.isNotEmpty ?? false,
  ...
  List _calculateFontSize(
    ...
    if (presetFontSizes == null) {
      final num defaultFontSize =
          (style?.fontSize ?? 1).clamp(widget.minFontSize, widget.maxFontSize);
      final defaultScale = defaultFontSize * userScale / (style?.fontSize ?? 1);
    ...
    while (left <= right) {
      ...
      double scale;
      scale = presetFontSizes == null
          ? mid * userScale * widget.stepGranularity / (style?.fontSize ?? 1)
          : presetFontSizes[mid] * userScale / (style?.fontSize ?? 1);
    ...
    double fontSize;
    fontSize = presetFontSizes == null
        ? right * userScale * widget.stepGranularity
        : presetFontSizes[right] * userScale;
  ...
  Widget _buildText(double fontSize, TextStyle? style, int? maxLines) {
    return widget.data != null
        ...
        : Text.rich(
            widget.textSpan ?? const TextSpan(),
            key: widget.textKey,
            style: style,
            strutStyle: widget.strutStyle,
            textAlign: widget.textAlign,
            ...
            softWrap: widget.softWrap,
            overflow: widget.overflow,
            textScaleFactor: fontSize / (style?.fontSize ?? 1),
            maxLines: maxLines,
            semanticsLabel: widget.semanticsLabel,
          );
  }
