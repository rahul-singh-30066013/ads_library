...
class DutyFreeDashBoardBuilder extends StatelessWidget {
  const DutyFreeDashBoardBuilder({
    Key? key,
  ...
  Widget build(BuildContext context) {
    ...
    switch (widgetType) {
      ...
      case WidgetType.image:
        return ContainerImageWidget(dashBoardItem: dashBoardItem);
      ...
      case WidgetType.gridScrollTileWithBorder:
        ...
        final width = screenWidth <= 375 ? 93.sp : 90.sp;
        const maxLines = 2;
  ...
  void _gridScrollTileAction(DutyFreeItem? data, BuildContext context) {
    if ((data?.restricted ?? false) &&
      ...
      context.read<DutyFreeState>()
        ..setFilters(
          data: (data?.code.isNotEmpty ?? false)
              ? data?.copyWith(materialGroup: data?.code ?? '')
              : data,
        )
                        ...
                        child: Container(
                          ...
                          child: Text(
                            ...
                            style: ADTextStyle500.size14
                                .setTextColor(
                                  context.adColors.blackTextColor.withOpacity(
                                      item.disableForAirport ? opacity : 1),
                                )
