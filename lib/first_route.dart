...
class _FirstRouteState extends State<FirstRoute> {
    ...
    return FutureBuilder(
      ...
      builder: (context, AsyncSnapshot<void> snapshot) {
        if (snapshot.connectionState == ConnectionState.done) {
          ...
          switch (widget.type) {
            case AdsType.dialog:
              final DialogBanner? adsDialogBanner =
                  value.adResponseState.data as DialogBanner?;
              return Center(
                child: ElevatedButton(
                    ...
                    onPressed: () => AdsDialogScreen()
                        .showDialogBanner(context, adsDialogBanner)),
              );
            case AdsType.singleBanner:
              final SingleBanner? singleBanner =
                  value.adResponseState.data as SingleBanner?;
              return Container(
                  ...
                  child: Image.network(
                      singleBanner?.fields?.first.imageLink ?? ''));
            case AdsType.bigBanner:
              final BigBanner? bigBanner =
                  value.adResponseState.data as BigBanner?;
              return Center(
                child: SizedBox(
                  height: 275.0,
                  child: ListView(
                    scrollDirection: Axis.horizontal,
                    children: List.generate(bigBanner?.fields?.length ?? 5,
                        (int index) {
                      return Container(
                        width: 220,
                        height: 240.0,
                            ...
                            child: Container(
                              ...
                              child: Column(
                                children: <Widget>[
                                  ...
                                  Image.network(
                                      bigBanner?.fields?[index].imageLink ??
                                          ''),
                                  //Image.network
                                  Text(
            ...
            case AdsType.smallBanner:
              final SmallBanner? smallBanner =
                  value.adResponseState.data as SmallBanner?;
              return Center(
                child: SizedBox(
                  height: 150.0,
                  child: ListView(
                    scrollDirection: Axis.horizontal,
                    children: List.generate(smallBanner?.fields?.length ?? 5,
                        (int index) {
                      return Card(
                        ...
                        child: SizedBox(
                          ...
                          height: 100.0,
                          child: Image.network(
                              smallBanner?.fields?[index].imageLink ?? ''),
                        ),
                      );
                    }),
                  ),
  ...
  Future<void> getData(BuildContext context) async {
    return widget.viewModel.fetchData(widget.type, context);
  }
}
